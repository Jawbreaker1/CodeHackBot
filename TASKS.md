...
The operator has since clarified that obtaining authorization is their responsibility, not an application intake gate. Intake and shared agent instructions now allow exploratory or formal runs to reach the normal proposal and selected approval mode without ownership challenges or a mandatory RoE questionnaire. Supplied authorization details remain part of the session/report record. The browser now shows brief coordinator conclusions and compact activity, plan, and worker-step previews; full technical text stays expandable. A persistent activity strip shows the coordinator and each active worker separately, including approval waits. A fresh Daybreak GUI conversation produced an external-target proposal without running tools or demanding owner paperwork; a separate local-only GUI assessment delegated one worker, showed both approval waits and evidence evaluation, completed, and opened in analysis. Saved context snapshots validated cleanly. The browser also retains model-request errors instead of clearing them on the next poll. This is a usability and product-policy correction, not proof of target-scope or deadline enforcement or multi-worker UI acceptance.
...
A same-question A/B check then completed three fresh Qwen 3.8 runs and three fresh Daybreak Blue runs on `example.com`: all six used DNS and one page fetch, retained about 3 KB of conversation context, and answered without an assessment proposal. One separate scan-request smoke per model produced an exact-host proposal without executing tools. Review found Qwen described cookie absence although response cookies were intentionally omitted from shared metadata; the observation now records only a Set-Cookie header count, without values, and the prompt limits absence claims to that response. Three further Qwen runs and one Daybreak run on the updated build used the count correctly. This validates the narrow intake distinction across both models; it does not validate security-scan quality or an air-gapped deployment.
...
- [ ] Validate the coordinator projection and worker packet under the configured Qwen 3.8 approximately 70k context/low-reasoning profile across multiple rounds; inspect real provider token usage, output/reasoning headroom, retained facts, and decision quality. A bounded synthetic inference reached the configured model with low reasoning and returned usage (58 prompt tokens, 47 completion tokens, including 35 reasoning tokens), but it is not an assessment-flow acceptance test. The updated Qwen application ceiling is 96 KiB of input text, not a token count; the earlier 48 KiB ceiling applied to the historical run. One small web-UI smoke request observed LM Studio loading a 70,144-token window with two slots and returned a normal reply; it did not stress VRAM or validate a long assessment. Audit how resource bounds are actually enforced. When the operator resumes Qwen testing, compare short and long-context requests at one and two concurrent slots; record GPU VRAM, host RAM/swap, time to first token, and generation speed. Treat host-memory spill or severe latency as a failed 70k configuration even if inference returns successfully. Further Qwen inference is paused at the operator’s 2026-09-26 request while the model server is used by another project; use Daybreak in the subscription bridge until they resume Qwen testing.
