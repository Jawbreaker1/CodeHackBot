...
2026-09-23 tool clarity: the shared model behavior frame now carries a phase-by-phase tool catalog. Intake is directed to propose a scoped worker task for commands or file changes instead of claiming the whole harness is read-only. Workers have a typed single-file `delete_file` decision routed through normal execution logging and approval, plus explicit guidance to keep user-requested individual changes separate. The local Qwen profile requests JSON-schema object output for structured control decisions because a live prose-only answer failed the intake contract. In a browser run with Qwen, intake described the phase-specific tools accurately, the worker presented a single-file deletion for approval, and denying it left both fixture files untouched. A second browser run after tightening the planning instruction proposed only the first file in round one. The runtime does not yet structurally enforce this sequencing if a model ignores the instruction; plan review remains the operator checkpoint. The approved deletion path is covered by a focused integration test; the browser runs intentionally did not execute a deletion.
