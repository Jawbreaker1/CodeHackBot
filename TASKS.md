...
- [ ] Revalidate the latest resource-limit and evaluator prompts with a new real-model end-user run and separately audit how resource bounds are actually enforced. The updated web build is serving locally; archive access is not proven by the two runs above.
