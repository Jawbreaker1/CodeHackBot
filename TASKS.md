...
- [ ] Revalidate the latest resource-limit and evaluator prompts with a new real-model end-user run and separately audit how resource bounds are actually enforced. The current web server still needs to be restarted to use the last prompt changes. Archive access is not proven by the two runs above.
