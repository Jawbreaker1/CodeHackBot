...
func Parse(s string) (Evaluation, error) {
	...
	if err := json.Unmarshal([]byte(cleaned), &e); err != nil {
		return Evaluation{}, fmt.Errorf("parse direct evaluation json: %w", err)
	}
	return e, nil
}
...
func Validate(e Evaluation) ValidationReport {
	...
	default:
		report.add("unsupported direct evaluation status")
	}
	if strings.TrimSpace(e.Reason) == "" {
		report.add("direct evaluation requires reason")
	}
	return report
}
