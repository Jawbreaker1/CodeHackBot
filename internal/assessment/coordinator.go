...
func coordinatorPayload(state State) coordinatorModelPacket {
	...
	return coordinatorModelPacket{
		...
		Instructions: []string{
			...
			"Treat every non-empty tasks array as a proposed sequence. Explain why each task matters through its goal and done_when; the runtime applies the operator's session approval mode before execution. Never treat an unselected task as completed evidence.",
