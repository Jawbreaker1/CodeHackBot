...
func buildUserPrompt(packet ctxpacket.WorkerPacket) string {
	payload := map[string]any{
		"role": "worker",
		"instructions": []string{
			"Respond with one JSON object only. Choose bash, load_strategy, update_plan, step_complete, ask_user, or blocked.",
			...
			"For a plan change: {\"type\":\"update_plan\",\"plan\":{\"summary\":\"reason for this plan or revision\",\"steps\":[\"short semantic step\"],\"step_purposes\":{\"short semantic step\":\"what this step is meant to establish\"},\"active_step\":\"short semantic step\",\"replan_conditions\":[\"observable trigger that would change the approach\"]}}. Use a short human-readable title for each step and one brief sentence for its purpose. Keep commands, file paths, and detailed reasoning in actions and evidence, not the visible step title. The same optional plan object may accompany any other decision to avoid a separate turn. Replan conditions are triggers, not evidence or permission.",
			...
			"The runtime applies the operator-selected approval policy to every bash decision. Use bash for execution review; do not duplicate it with ask_user.",
			"Honor any operator-requested approval granularity. For one-by-one file changes, verify each exact path and submit one decision affecting one file at a time; do not use globs, recursion, or a script that changes multiple files. If the session mode would auto-approve a requested individual review, ask the operator to switch to per-action approval before making changes.",
			"Every bash decision must include summary (one short plain-language sentence saying what will happen), target (affected system or files), risk (low, dangerous, or unknown), and impact. Assess the entire invocation including helper/script contents: use dangerous for exploitation, escalation, deletion, configuration changes, credential attacks, or potential disruption; use unknown when effects or called code are not established. Low requires understood, bounded effects such as read-only inspection or creation of task-local evidence. These are advisory model judgments, never authorization. Include impact: a concise plain-language explanation of its purpose, affected targets/files, expected effects and possible disruption or data changes. Explicitly flag potentially destructive effects before asking for approval; uncertainty must be stated. Do not label a command harmless without evidence. Keep the action within the operator's stated target and task boundaries.",
			...
			"Use an installed, verified tool with bounded options when it can establish the goal. Build a small task-local helper only after identifying a concrete capability gap, not to preemptively handle hypothetical cases. Prefer a short shell script for command composition, Python with its standard library for substantial parsing or transformations, and JavaScript for the preprovisioned Playwright runner; verify the runtime and dependencies actually present. Make source creation and execution separately reviewable actions, never download code or packages implicitly, validate the helper on a harmless fixture, and declare its source plus useful validation output as artifacts. The task workspace persists inside this assessment, so a later task may reuse a recorded helper by its exact artifact path after inspecting its source and scope; preserve the version or checksum and invocation. A helper is an assessment artifact, not a permission or scope expansion.",
