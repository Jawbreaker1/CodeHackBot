...
func (l Loop) Run(ctx context.Context, packet ctxpacket.WorkerPacket, maxSteps int) (out Outcome, runErr error) {
	...
	for current.Budget.Used < current.Budget.Limit {
		...
		case "action", "delete_file":
			if response.Type == "delete_file" {
				response.Type = "action"
				response.Command = "rm"
				response.Args = []string{"--", response.Path}
				response.Target = response.Path
				response.Risk = "dangerous"
			}
...
func buildUserPrompt(packet ctxpacket.WorkerPacket) string {
	payload := map[string]any{
		"role": "worker",
		"instructions": []string{
			"Respond with one JSON object only. Choose action, delete_file, load_strategy, update_plan, step_complete, ask_user, or blocked.",
			"To keep one relevant local guide in this worker's context across later turns: {\"type\":\"load_strategy\",\"strategy\":\"relative/path/from/catalog.md\"}. Choose it from the local strategy catalog in the behavior frame when it can improve an unfamiliar or failed approach. A coordinator-suggested path in capability_inputs is a lead, not a required choice; change guides if observations call for it. The runtime reads only a bounded Markdown file within that catalog directory and records its path and SHA-256; it is supporting knowledge, not target evidence or permission. Do not use an action just to read a strategy guide, and do not load unrelated guides.",
			...
			"For deletion of one verified file: {\"type\":\"delete_file\",\"path\":\"/clean/absolute/path\",\"summary\":\"plain-language description\",\"impact\":\"what will be removed and why\"}. This invokes one non-recursive file removal through the same approval and evidence path as action. It cannot remove a directory or accept a second path. Use it when an operator requests separate approval for each file; never bundle several deletions in a shell action.",
			...
			"The runtime applies the operator-selected approval policy to action and delete_file. Use those decisions for execution review; do not duplicate it with ask_user.",
