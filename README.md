...
The initial browser surface is available through `birdhackbot-web`. It is chat-first and multimodal: the shared intake protocol lets the coordinator explain the harness, resolve exploratory discovery with bounded read-only observations (including fixed host identity metadata), clarify scope, and propose an assessment before the operator sees a compact review card. A bounded, non-intrusive inspection of an exact external target can start from the operator's authorization statement and reviewed scope, without a separate RoE form; broader testing uses a formal RoE. The conversation and proposal are saved with the session. After starting, the coordinator proposes a bounded test sequence in the main conversation; the operator can select or reject tasks before workers execute. Workers remain visible while the operator continues talking to the coordinator. Typed observations such as directory listings appear as readable cards with expandable raw evidence. The browser uses the same coordinator and worker runtime as the terminal UI, keeps scope and approvals explicit, groups multiple assessment sessions under a customer workspace, and lets you drag an unclassified draft conversation into an existing customer workspace before starting it.
