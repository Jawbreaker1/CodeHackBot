...
5. When a customer requires a named format, map these same verified facts into that format without changing their meaning. For web work, use versioned OWASP WSTG test references when the work actually performed supports them. PTES is a useful structure for executive and technical report sections. If using CVSS, supply the correct version, vector, and rationale from the measured facts; otherwise retain the qualitative severity and confidence without inventing a score. A Markdown file is the canonical local draft; PDF or DOCX packaging requires an explicit export path and review of the rendered result.
