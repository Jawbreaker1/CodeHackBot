...
Before first execution, model-led intake can propose three investigation depths with preliminary elapsed-time ranges. The operator's selection is saved in the assessment and guides planning; it neither expands scope nor guarantees a deadline. A request to investigate or scan a named target with no current evidence should yield a minimal exploratory proposal, rather than an authorization questionnaire or a menu of possible goals. All depth choices must stay within the same target and allowed-action boundaries; depth increases coverage, not scope. The operator is responsible for authorization. Intake does not verify ownership or require an authorization statement, RoE document, storage path, testing window, or escalation contact before proposing a run; it asks only when the requested target or action is genuinely unclear. An operator-named hostname remains the exact hostname in the proposal; DNS resolution does not add related hostnames to scope. The durable transcript records any authorization and formal engagement details the operator supplies. Each coordinator decision supplies a short plain-language purpose and, after round one, a review of what the preceding worker evidence established. Both interfaces show concise updates in the conversation; complete technical conclusions, tasks, raw activity, and evidence remain available behind expandable detail or in reports.

Simple information requests about a named public site use the shared intake observation path in connected mode: public DNS lookup and one bounded HTTP/HTTPS GET, each subject to the session approval level and saved as evidence. A failed lookup or one fetched page does not establish site-wide security or the reason for a DNS failure. A request to investigate, scan, or validate security instead moves to an assessment proposal and worker plan. The external observation path is unavailable in air-gapped mode. Both terminal and web clients use this same intake behavior.
...
Execution results preserve invocation, mode, cwd, timestamps, exit status, output previews, log references, and output artifact references. Stdout and stderr stream to separate local files while the tool runs. Previews are bounded to 8 KiB per stream and explicitly indicate truncation. The completed log includes the full output, copied without buffering it all in memory. A browser coordinator reply may carry typed references to a few already-registered image artifacts. The server validates each against the assessment's evidence and session directory, then renders local image URLs beneath that reply; model-authored prose is never interpreted as a filesystem path or HTML image source.
...
Recent operator messages preserve line breaks and indentation. Live coordinator replies receive a bounded, ordered tail of the saved conversation alongside current assessment state and the newest operator message; older turns are omitted with an explicit marker, and the durable transcript remains intact. Older conversation notes are bounded excerpts, not authoritative semantic memory. Deep copies isolate UI snapshots and compact model views from mutable execution state. Long investigations still require evaluation of retrieval quality and model-specific context sizing.
...
budget. The local Qwen profile currently caps input text at 96 KiB and requests
...
The web analysis view is a read model over assessment state, separate from the coordinator transcript. It aggregates sessions by customer, ranks findings using the reported severity, confidence, and candidate/reproduced status, shows why each item is prioritized, and lists next actions and gaps. Candidate leads are counted separately from validated severity totals. This ranking is deliberately transparent and does not pretend to be CVSS scoring. The Markdown report includes the selected test sequence, findings, advisory references, evidence references, remediation, and assessment gaps. If an assessment ends before the coordinator reviews its latest worker result, Analysis shows that result separately, and reports use an explicit incomplete summary rather than presenting the preceding plan as a final conclusion. The formal template omits the unreconciled plan's gap list while recording the run limitation; the canonical report retains the historical plan and full evidence. These drafts require operator review; a formal report is not a guarantee that no vulnerability exists.

Completed browser assessments continue to accept coordinator questions. For an explicit OWASP WSTG or PTES report request, the coordinator emits a structured format choice, and the server renders a new Markdown artifact from saved assessment state using a versioned template. The artifact is stored under the assessment's `reports/` directory and linked in the transcript. The canonical `report.md` and assessment evidence remain unchanged. Post-run conversation has a separate 24-call model budget, recorded in session metadata; it cannot execute worker tasks. A template fixes the report's structure, not the truth of its model-authored findings or a claim of standards certification.
