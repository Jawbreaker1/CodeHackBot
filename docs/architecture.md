...
Before first execution, model-led intake can propose three investigation depths with preliminary elapsed-time ranges. The operator's selection is saved in the assessment and guides planning; it neither expands scope nor guarantees a deadline. An exact external target may enter a bounded, non-intrusive inspection from the operator's authorization statement and a reviewed proposal; the durable intake transcript and copied assessment transcript retain that statement. Broader customer testing requires the full RoE in `AGENTS.md`. Intake asks only for missing facts and never asks the operator to provide a record storage path. Each coordinator decision supplies a short plain-language purpose and, after round one, a review of what the preceding worker evidence established. Both interfaces show these updates in the conversation; detailed tasks, raw activity, and evidence remain available for inspection.
