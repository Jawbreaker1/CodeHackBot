...
The current implementation covers bounded projections, protected anchors, conversation rollover, plan history, visible truncation, and separate compact projections for coordinator and worker handoffs. Oversized latest worker output, older output bodies, and superseded plan details are now excerpted or removed from model requests before the hard limit is reached; the durable records and their references remain intact. Coordinator result cards carry recent bounded previews, while the complete registered evidence catalog remains available for exact citations. These are navigation summaries, not semantic compression of all past observations. The inspector meters the largest active worker request, or the most recently updated worker after completion, and names the source; it is not a provider token count. The next context increments are relevance-ranked retrieval across a run, a durable multi-worker event history, and independently verified run summaries. Each should be added only with a fixture that demonstrates the information loss it prevents; context size alone is not a reason to add another compaction layer.
