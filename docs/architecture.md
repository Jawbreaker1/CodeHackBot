...
Before first execution, model-led intake can propose three investigation depths with preliminary elapsed-time ranges. The operator's selection is saved in the assessment and guides planning; it neither expands scope nor guarantees a deadline. The operator is responsible for authorization. Intake does not verify ownership or require an authorization statement, RoE document, storage path, testing window, or escalation contact before proposing a run; it asks only when the requested target or action is genuinely unclear. An operator-named hostname remains the exact hostname in the proposal; DNS resolution does not add related hostnames to scope. The durable transcript records any authorization and formal engagement details the operator supplies. Each coordinator decision supplies a short plain-language purpose and, after round one, a review of what the preceding worker evidence established. Both interfaces show concise updates in the conversation; complete technical conclusions, tasks, raw activity, and evidence remain available behind expandable detail or in reports.
