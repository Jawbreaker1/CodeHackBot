...
## Customer and session workflow

Enter a stable customer workspace ID, exact objective, and exact scope in the browser. Each create request produces a separate assessment session below `sessions/<customer>/<session-id>`. Start remains an explicit review action. Workers show progress and pending approvals in the browser; the operator can approve, deny, answer a coordinator question, send a message, or stop the assessment.
