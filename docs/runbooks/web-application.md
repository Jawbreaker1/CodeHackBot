...
The browser opens a model-led intake conversation. The composer can attach up to four bounded PNG, JPEG, WebP, GIF, or PDF artifacts. Attachments are stored locally under the session with restrictive permissions, shown in the transcript, and sent as typed visual/file input only for the current model turn; do not attach credentials or unrelated customer data. The subscription bridge forwards images and PDFs to the Responses model path. Local endpoints must support the corresponding multimodal format; otherwise the coordinator reports the provider limitation. Ask about the harness, describe a security question, or provide partial context. The coordinator can request a bounded, read-only observation of the configured workspace, fixed host identity metadata, or this host's network metadata when that resolves the question; the browser shows the exact request and waits for approval. Directory and host results are rendered as typed summaries with expandable raw evidence. It can also guide an exploratory assessment without forcing the operator to know a CIDR before discovery. The operator owns authorization; the coordinator does not demand proof or RoE details before proposing work. It asks focused questions only when the target or requested action is genuinely unclear. The saved conversation and proposal retain any formal engagement details the operator provides. Enter a stable customer workspace ID in the review card and start the assessment under the selected approval level. The initial screen does not ask the operator to fill out an assessment form. During an assessment, workers may build a small helper when needed, but source creation, dependency use, execution, and validation remain separate visible approval steps with local evidence.

In connected mode, a plain question about a named public site can use the shared intake DNS and bounded page-fetch tools and receive an answer without starting an assessment. The selected approval level applies to each observation; page evidence includes selected response headers and the count of Set-Cookie headers, not cookie values. A DNS failure or one page fetch is not a site-wide security conclusion. External intake observations are unavailable in air-gapped mode.
...
When a worker records a PNG, JPEG, WebP, or GIF artifact, its exact path can be offered to the coordinator in the live assessment state. A coordinator chat reply may select up to three of those references for inline display beneath its answer. The server verifies that each selected file is a recorded image inside this assessment before returning a session-scoped artifact URL; invalid paths are ignored. The selection and original artifact paths persist with the chat message so the same images appear after reopening the session. This does not upload screenshots to an external image host.
...
The preview still has no authentication, origin/CSRF protection, or streaming transport. Keep the browser server on loopback; remote deployment remains out of scope until those controls exist. Assessment target authorization follows `AGENTS.md`.
