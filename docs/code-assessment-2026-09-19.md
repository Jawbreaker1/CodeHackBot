...
Historical baseline assessment at checkpoint `95edae1`. Core cleanup has since addressed several findings; see `TASKS.md` for current status. Source links below are pinned to the assessed code.
...
- A1/A2: [action preparation](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/workerloop/actionprep.go#L25), [implicit shell selection](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/execx/executor.go#L165), and [approval before execution planning](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/workerloop/loop.go#L293). Approval uses the original `UseShell` flag; the later executor derives its own mode. These are boundary defects, not problems a better prompt can reliably fix.
- A3: [process configuration](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/execx/noninteractive_unix.go#L10) creates a session, while [command construction](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/execx/executor.go#L165) uses default `CommandContext` cancellation without descendant teardown or bounded pipe draining.
- A4/A5: [direct success shortcut](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/workerloop/directsemantics.go#L79) and [planned completion fallback](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/workerloop/stepsemantics.go#L45) accept evidence without establishing its relevance to the active objective.
- A6: [truth ranking](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/context/truth.go#L16) ranks old failures above new successes. [New task construction](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/interactivecli/shell.go#L921) carries previous results without task identifiers or a relevance decision. Always preferring the newest result would also be wrong; evidence needs identity, relevance, and explicit supersession.
- A7: [scope normalization](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/workerloop/planner.go#L171) and [private-address/path shortcut](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/workerloop/planner.go#L207). The [session foundation](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/session/foundation.go#L9) contains only a goal and reporting requirement. Supply explicit engagement boundaries and enforce them through the execution environment; avoid a command-name or text-inference policy engine.
- A8: [packet validation](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/context/validation.go#L41) tests rendered prompt text, but [rendering](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/behavior/frame.go#L48) always adds headings. The existing missing-goal-and-behavior test passes because the missing goal supplies the fatal error.

## Additional gaps established by inspection

**Evidence and persistence need a different foundation.** [Execution](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/execx/executor.go#L86) buffers stdout/stderr without a size limit and writes their contents only after the process finishes. The initial log records that execution started, but does not stream output. Summaries retain only the first five lines; result heuristics inspect those summaries. Full logs exist after normal completion, but that is insufficient for long-running tools, crashes, and large outputs. Artifact references are always nil at this boundary. The worker's reduced result drops invocation, environment, and timing fields; its result type has no run/task/execution identity.

[Session persistence](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/sessionstate/state.go#L27) directly overwrites a JSON file. There is no atomic replacement or durable event journal. Crash corruption and recovery behavior were not fault-injected in this review. Context packets currently mix durable task state, selected evidence, prompt text, and UI state. Adding concurrent workers to this structure would make ownership and recovery harder.

**The product's main capabilities do not exist in the active runtime yet.** The [orchestrator entrypoint](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/cmd/birdhackbot-orchestrator/main.go#L20) is a placeholder. There is no implemented source-to-deployment correlation pipeline or structured findings/report service in the active packages. These are new implementation work, not features unlocked by renaming the CLI.

**Provider support is a prototype boundary.** The [client](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/llmclient/client.go#L14) is a concrete chat-completions client with fixed request assumptions. It has no built-in authentication or streaming, capability negotiation, rate-limit coordination, or aggregate budget enforcement. It preserves usage in its normalized response, which is worth retaining as an idea. Provider-specific reasoning-text fallback should not be a generic machine-action contract. The subscription bridge needs a bounded provider experiment before choosing its implementation; an embedded agent service and a raw inference service have different execution ownership.

**The UI must become a client of the runtime.** Ctrl-C [immediately quits the TUI](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/internal/interactivecli/bubble.go#L184) after canceling its context; worker finalization is not awaited there. Task preparation can call the model synchronously from the UI update handler. Both the TUI and the [approval reader](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/cmd/birdhackbot/main.go#L300) use stdin. These paths need integrated lifecycle/PTY tests; this assessment did not reproduce a terminal-input failure. Keep the display work, but move task ownership, approvals, and stop/finalization into the runtime.

**Evaluation capture is stale.** [The repeat harness](https://github.com/Jawbreaker1/CodeHackBot/blob/95edae1254461310584e94f6ae8c9a2a977f704d/scripts/repeat_worker_run.sh#L91) does not pass a unique session directory, then copies from `sessions/rebuild-dev`. It can collect missing or stale session data. Repair artifact attribution before using it to compare old and new behavior. More coverage alone will not resolve these contract gaps.
